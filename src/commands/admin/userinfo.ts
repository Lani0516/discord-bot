...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: '帳號建立日期', value: `<t:${Math.floor(user.createdTimestamp / 1000)}:R>`, inline: true },
      { name: '加入伺服器日期', value: member ? `<t:${Math.floor(member.joinedTimestamp! / 1000)}:R>` : '未知', inline: true },
      { name: '所屬身分組', value: roles },
    )
    .setTimestamp();
