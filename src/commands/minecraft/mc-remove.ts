...
import { getMcServer, removeMcServer } from '../../database.ts';
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (!config) {
    return interaction.reply({ content: '此伺服器尚未設定 Minecraft 監控功能，無法執行移除操作。', ephemeral: true });
  }
  ...
  if (config.message_id && config.channel_id) {
    try {
      const channel = await interaction.client.channels.fetch(config.channel_id) as TextChannel;
  ...
  await interaction.reply({ content: 'Minecraft 伺服器監控已成功移除，狀態訊息將不再自動更新。', ephemeral: true });
}
